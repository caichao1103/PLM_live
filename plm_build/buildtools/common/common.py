...
def run_command(cmd, cwd=None, is_print=True, encrypt=None):
    ...
    if exit_code != 0:
        log_tool.show_error('Run Command Error for {}, Error code: {}'.format(cmd_str, exit_code))
        exit(exit_code)
...
def is_exists(path):
    if os.path.exists(path):
        log_tool.show_info('{} exists !'.format(path))
        return path
    else:
        return is_exists(os.path.dirname(path))